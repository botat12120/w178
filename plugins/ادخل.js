...
const handler = async (m, {conn, text, isMods, isOwner, isPrems}) => {
 if (enviando) return;
  ...
  try {
    ...
    if (!link || !link.match(linkRegex)) throw '*[❗] خطأ حط رابط القروب الي تبي البوت فيها.*\n\n*—◉ مثل:*\n*◉ .انضم https://chat.whatsapp.com/Ck3mQdmm0Ty3FMrJSNsG90
*';
    const [_, code] = link.match(linkRegex) || [];
    if ( isPrems || isMods || isOwner || m.fromMe) {
      const res = await conn.groupAcceptInvite(code);
      await conn.sendMessage(m.chat, {text: '*[ ✔️ ] تم الانضمام *'}, {quoted: m})
      enviando = false 
    } else {
      conn.sendMessage(m.chat, {text: '*[❗] تم الارسال الى المطور انتظر حتي يتم الموافقة على مجموعتك*\n\nهناك احتمالات ان لا يتم الموفقه علي المجموعه\nلقد دفعت الكثير من العملات لكن سوف يتم التحقق الاول من مجموعتك\n*'}, {quoted: m});
